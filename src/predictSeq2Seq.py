...
if __name__ == '__main__':
    ...
    encoder1 = EncoderRNN(len(embedding.vocab), hidden_size, embedding.vectors, BATCH_SIZE).to(device)
    encoder1.load_state_dict(torch.load(encoderName))
    decoder1 = DecoderRNN(hidden_size, len(embedding.vocab), embedding.vectors, BATCH_SIZE).to(device)
